import { login } from "@/app/features/users";
...
export default function Login() {
  ...
  function handleSubmit(e) {
    e.preventDefault();
    dispatch(login({email, password,itemsInCart}))
  }
...
if(isLoggedIn){
  ...
  return (
    <>
      <div className="min-h-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 font-primary relative overflow-hidden">
        <div ref={notifyRef} className="absolute top-0 right-0 py-4 px-4 w-80 h-fit">
          {loading && <Notification
            border="border-blue-500"
            text="text-blue-600"
            title="Logging"
            desc="You are being authenticated"
            bg="bg-blue-100"
          />}
          {error && <Notification
